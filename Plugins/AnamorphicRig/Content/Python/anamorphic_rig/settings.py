...
class AnamorphicRigSettings(unreal.Object):
    ...
    fit_preview = unreal.uproperty(bool, meta=_m("05 출력", 90,
        DisplayName="한 화면에 맞춰 축소",
        ToolTip="켜면 벽 전체를 모니터 한 대에 담는다. 창을 벽 순서대로 붙여 놓고, "
                "주 모니터보다 크면 비율을 지켜 줄인다. 화소가 줄어드니 눈으로 훑는 용도다. "
                "끄면 원본 해상도 그대로다. 멀티 노드면 창을 모니터에 하나씩 놓고, "
                "아니면 화면 원점부터 놓는다. 현장 배치이자 선명도를 판단할 수 있는 상태다"))
